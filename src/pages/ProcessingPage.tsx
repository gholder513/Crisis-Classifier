...
import { AlertTriangle, ArrowLeft, BookOpen, Brain, ChevronDown, Search } from 'lucide-react';
...
const ProcessingPage: React.FC = () => {
  ...
  const [classifier, setClassifier] = useState<ClassifierModel>(createClassifier());
  const [isLoading, setIsLoading] = useState(false);
  const [message, setMessage] = useState<{ text: string; type: 'info' | 'success' | 'error' } | null>(null);
  const [activeTab, setActiveTab] = useState<'articles' | 'training'>('articles');
  ...
  const trainWithUserData = () => {
    ...
    try {
      ...
      if (!hasCrisisArticles) {
        showMessage('Please mark at least one article as a crisis event for training', 'error');
        setIsLoading(false);
        return;
      }
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        <div className="bg-white shadow rounded-lg overflow-hidden">
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">Article Processing</h2>
                <p className="text-gray-600">Add, classify, and manage crisis articles</p>
              </div>
              <div className="flex items-center gap-3">
                <div className={`w-3 h-3 rounded-full ${classifier.isReady ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                <span className="text-sm text-gray-600">
                  {classifier.isReady ? 'Model is trained and ready' : 'Model needs training'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
            {activeTab === 'articles' ? (
              <>
                <div className="mb-6">
                  <h3 className="text-lg font-medium mb-4">Add Article URL</h3>
                ...
                <div className="mt-8">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-medium">Articles ({articles.length})</h3>
                    <div className="flex items-center gap-2">
                      <Search size={16} className="text-gray-400" />
                      <input
                        type="text"
                        placeholder="Search articles..."
                        className="text-sm border-none focus:ring-0 text-gray-600 bg-gray-50 rounded-md"
                      />
                    </div>
                  </div>
              ...
              <div className="py-4">
                ...
                <div className="mt-8 bg-gray-50 p-6 rounded-lg border border-gray-200">
                  <h3 className="text-lg font-medium mb-4">Training Information</h3>
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">
                        {articles.filter(a => a.isCrisis).length} crisis articles and {articles.filter(a => a.isCrisis === false).length} non-crisis articles available for training.
                      </p>
                    </div>
                    
                    <div>
                      <h4 className="font-medium text-gray-700">Classification Results</h4>
                      <p className="text-gray-600">
                        {articles.filter(a => a.prediction !== undefined).length} articles classified: {articles.filter(a => a.prediction === true).length} crisis events detected.
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">Total Articles</h3>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">Crisis Events</h3>
            <p className="text-3xl font-bold text-red-600">{articles.filter(a => a.prediction === true).length}</p>
            <div className="mt-2 flex items-center text-sm text-gray-600">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">Model Confidence</h3>
            <p className="text-3xl font-bold text-purple-600">
              {articles.filter(a => a.confidence !== undefined).length > 0
                ? `${(articles.reduce((sum, article) => sum + (article.confidence || 0), 0) / 
                    articles.filter(a => a.confidence !== undefined).length * 100).toFixed(1)}%`
                : 'N/A'}
            </p>
            <div className="mt-2 flex items-center text-sm text-gray-600">
        ...
        <div className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="flex items-center gap-2 mb-4 md:mb-0">
              <AlertTriangle size={20} className="text-purple-800" />
              <span className="text-gray-600">Crisis Classifier</span>
            </div>
